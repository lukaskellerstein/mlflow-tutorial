...
```bash
cd tutorial/level_1/M3_tracing/1_autologging
uv sync
...
1. Navigate to the experiment **L1/M3_tracing/1_autologging**
...
In **L1-M3.2 (Manual Tracing)**, you will learn how to add custom tracing with `@mlflow.trace` and `mlflow.start_span()` for business logic that autologging doesn't cover.
