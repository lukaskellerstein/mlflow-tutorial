...
In this lesson we use the same small model (`gemma4:e2b`) for both student and judge to keep things fast. In production you would typically use a larger, more capable model as the judge (e.g., `gemma4:26b` or a frontier model).
...
```python
student = ChatOllama(model="gemma4:e2b", temperature=0.3)
...
```python
judge_llm = ChatOllama(model="gemma4:e2b", temperature=0.0)
...
with mlflow.start_run(run_name="llm_as_judge_eval"):
    mlflow.log_param("student_model", "gemma4:e2b")
    mlflow.log_param("judge_model", "gemma4:e2b")
