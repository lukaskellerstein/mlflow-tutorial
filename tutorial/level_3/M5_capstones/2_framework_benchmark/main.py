...
mlflow.set_experiment("L3/M5_capstones/2_framework_benchmark")
...
def build_stategraph_agent() -> callable:
    ...
    def process_math(state: GraphState) -> dict:
        ...
        q = user_msg.content if isinstance(user_msg, HumanMessage) else str(user_msg)
        resp = LLM.invoke([
            SystemMessage(content="Extract ONLY the math expression from this question. "
    ...
    def respond_node(state: GraphState) -> dict:
        ...
        prompt_parts = [SystemMessage(content="Answer concisely in 1-2 sentences.")]
        if ctx:
