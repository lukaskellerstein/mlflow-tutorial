...
- Completed: L2-M1.1.1, L2-M1.1.2, and L1-M2.1 (auto vs manual tracing)
...
- **This lesson makes real Anthropic API calls and costs real money** — roughly
  ...
  that does; the other two go through the local MLflow AI Gateway.
...
MLflow AI Gateway from `infra/` speaks the OpenAI API. They do not meet, so this
...
```bash
cd tutorial/level_2_agents/M1_agent_frameworks/1_turn/3_claude_agent_sdk
uv sync
...
In the MLflow UI under **L2/M1_agent_frameworks/1_turn/3_claude_agent_sdk**: one parent
