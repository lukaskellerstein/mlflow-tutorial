# L2-M3.2 — GAIA General Assistant Benchmark
...
- Completed: L2-M3.1 (SWE-Bench) for the benchmark harness and SDK tracing patterns
...
The agent is locked to exactly these tools: `tools=[]` disables the built-ins, `strict_mcp_config=True` blocks ambient MCP servers, and `setting_sources=[]` keeps local settings out of the context (see L2-M3.1 for why each matters).
...
```bash
cd tutorial/level_2_agents/M3_agent_benchmarks/2_gaia
uv sync
...
In L2-M3.3, you'll learn to design your own domain-specific agent benchmark from scratch — defining task taxonomies, creating evaluation datasets, and building reusable harness patterns.
