...
- Completed: L2-M2.1 (Agent Test Generation and Simulation), L1-M4.1 (Evaluation
  Fundamentals)
...
```bash
cd tutorial/level_2_agents/M2_agent_evaluation/2_judges
uv sync
...
In the MLflow UI, experiment `L2/M3_agent_evaluation/2_judges` holds 12 traces
...
- `make_judge` produces judges as *data*, which is what makes them registrable —
  and registration is the gate to everything in L2-M2.6.
...
**L2-M2.3 (Agent Quality Metrics and Session Scorers)** moves from one rubric to
...
can express. The registered judge you built here is reused in L2-M2.4 and goes
online in L2-M2.6.
