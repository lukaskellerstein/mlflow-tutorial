...
- Completed: L2-M2.1 (Test Generation and Simulation), L2-M2.2 (Judges), L1-M4.1 (Evaluation Fundamentals)
...
```bash
cd tutorial/level_2_agents/M2_agent_evaluation/3_quality_metrics
uv sync
...
In the MLflow UI, experiment `L2/M2_agent_evaluation/2_quality_metrics` holds the
...
**L2-M2.4 (Agent Architecture Comparison)** reuses this metric suite to compare
agent architectures under a controlled methodology, scoring through
`mlflow.genai.evaluate()` with the registered judges from L2-M2.2 so results stay
