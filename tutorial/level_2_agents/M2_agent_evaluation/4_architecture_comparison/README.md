...
- Completed: L2-M2.1 (Test Generation), L2-M2.2 (Judges), L2-M2.3 (Quality Metrics)
...
stays meaningful outside the script that produced it, and L2-M2.6 can put the
...
`@scorer` cannot be registered against an open-source server anyway (L2-M2.2).
...
```bash
cd tutorial/level_2_agents/M2_agent_evaluation/4_architecture_comparison
uv sync
...
- Registering the judge is what makes scores portable — the same named, versioned
  object scores every architecture, later runs, and (in L2-M2.6) production.
...
**L2-M2.5 (Agent Optimization)** takes an architecture and systematically
