...
# win and every judge call would be rejected by the gateway. See L2-M2.1.2.
os.environ["OPENAI_API_KEY"] = GATEWAY_KEY
os.environ["OPENAI_BASE_URL"] = GATEWAY_URL

mlflow.set_tracking_uri("http://127.0.0.1:5555")
mlflow.set_experiment("L2/M2_agent_evaluation/2_offline/1_architecture_comparison")
...
#
# Correctness is therefore judged by a REGISTERED judge (L2-M2.1.2), not a local
...
# meaningful outside the script that produced it, and L2-M2.2.2 can put the very
...
def tool_usage(outputs: dict, expectations: dict) -> Feedback:
    ...
    cannot be registered against an open-source server anyway (see L2-M2.1.2).
    """
...
def main() -> None:
    print("=" * 70)
    print("  L2-M2.2.1 — Agent Architecture Comparison")
