...
- Completed: the L2-M2.1 instruments (the test material), L2-M2.1.1.2 (Judges), L2-M2.1.1.3 (Turn-Level Quality Metrics)
...
stays meaningful outside the script that produced it, and L2-M2.2.1.2 can put the
...
`@scorer` cannot be registered against an open-source server anyway (L2-M2.1.1.2).
...
```bash
cd tutorial/level_2_agents/M2_agent_evaluation/2_offline/1_turn/1_architecture_comparison
uv sync
...
- Registering the judge is what makes scores portable — the same named, versioned
  object scores every architecture, later runs, and (in L2-M2.2.1.2) production.
