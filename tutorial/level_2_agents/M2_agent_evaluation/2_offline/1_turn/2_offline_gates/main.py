...
OFFLINE evaluation answers one question: "is this version good enough to ship?"
A curated dataset, known expectations, every case scored, and YOU decide when it
runs.

  Part 1: the full pipeline (dataset -> agent -> score -> quality gates -> report)
  Part 2: regression detection against a stored baseline

It cannot answer "is what shipped still good?" -- production traces carry no
expectations, coverage has to be sampled, and the server pulls the trigger. That
is the other mode, and it has its own lesson: L2-M2.3.1.1 (Online Scoring).

Follows the L2-M2.1 instruments, which supply the test material.

NOTE WHAT THIS LESSON DELIBERATELY DOES NOT USE. There is no `@scorer`, no
`make_judge` and no `mlflow.genai.evaluate` here. `score_results()` is plain
Python: a substring match, a tool-name match, and a latency average.

That is a choice, not an omission. A gate produces a DECISION -- ship or block --
and a judge is a model, so a judge in the gate makes the decision vary between
runs on the same candidate. Here the same inputs give the same verdict every
time, which is what lets Part 2 read a delta against a baseline and call it a
regression rather than noise.

The cost is that a substring match fails on correct answers phrased differently.
When that starts blocking good builds, swap in a judge from L2-M2.1.1.2 and
accept the variance -- 2_conversation/1_conversation_gates is the worked example
of a gate built on judges, and of what you have to do about the noise.
"""
...
mlflow.set_experiment("L2/M2_agent_evaluation/2_offline/1_turn/2_offline_gates")
...
def main() -> None:
    ...
    print("  Experiment: L2/M2_agent_evaluation/2_offline/1_turn/2_offline_gates")
    print("=" * 60)
    print("\n  This answered 'good enough to ship?'. For 'is what shipped still")
    print("  good?', continue to L2-M2.3.1.1 — Online Scoring on Production Traces.")
