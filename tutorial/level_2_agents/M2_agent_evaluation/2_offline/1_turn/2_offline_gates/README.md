...
- Completed: the L2-M2.1 instruments (the test material)
- **Not** required: L2-M2.1.1.2 (Judges). This lesson scores with plain Python on
  purpose — see "Why no judge here" below
...
[L2-M2.3.1.1](../../../3_online/1_turn/1_online_scoring/), and the contrast is the reason M2
...
There is no `@scorer`, no `make_judge` and no `mlflow.genai.evaluate` in this
lesson. `score_results()` is a substring match, a tool-name match and a latency
average — and that is a choice, not an omission.

A gate produces a **decision**, and a judge is a model. Put a judge in the gate
and the same candidate can ship on Monday and be blocked on Tuesday. Here the
same inputs give the same verdict every time, which is exactly what lets Part 2
read a delta against the baseline and call it a **regression** rather than noise.

The cost is real: a substring match fails on a correct answer phrased
differently. When that starts blocking good builds, swap in a judge from
L2-M2.1.1.2 and accept the variance.
[`2_conversation/1_conversation_gates`](../../2_conversation/1_conversation_gates/)
is the worked example of a judge-based gate, and of what you have to do about
the noise it brings.
...
def build_agent():
    llm = ChatOpenAI(
        model="gemma-chat",  # a gateway alias, not a provider model id
...
```bash
cd tutorial/level_2_agents/M2_agent_evaluation/2_offline/1_turn/2_offline_gates
uv sync
