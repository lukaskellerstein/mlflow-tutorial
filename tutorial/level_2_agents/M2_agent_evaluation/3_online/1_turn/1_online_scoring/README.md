...
- Completed: L2-M2.1.1.2 (Judges) — registered vs. inline judges
- Completed: L2-M2.2.1.2 (Offline Gates) — the other half of the question
...
- **`UNSLOTH_API_KEY` exported in your shell** — the only key this stack needs, and the MLflow gateway secret is created from the environment, never from a file in this repo
...
`start()` refuses any judge whose model is not a **gateway** model. Scoring runs inside the MLflow server, on its own schedule, long after your script has exited — so it cannot borrow the base URL the agent uses. That is why the judge names `gateway:/gemma-judge`, which the server resolves against its own database. Nothing has to be built: `infra/mlflow/gateway/seed_gateway.py` defines that endpoint and the stack seeds it on every `up -d`.
...
```bash
cd tutorial/level_2_agents/M2_agent_evaluation/3_online/1_turn/1_online_scoring
uv sync
