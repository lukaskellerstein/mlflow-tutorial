...
```bash
cd tutorial/level_1_models/M4_evaluation/1_evaluation_fundamentals
uv sync
...
In the MLflow UI, navigate to experiment **L1/M4_evaluation/1_evaluation_fundamentals** to see all logged metrics and evaluation runs.
...
Continue to **L1-M4.2 (GenAI Custom Metrics)** to learn about `make_metric()` for custom metric functions, the `Feedback` object for rich scorer metadata, and batch evaluation across multiple LLM configurations.
